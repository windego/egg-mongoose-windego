...
module.exports = app => {
  const { client, clients, url, options, defaultDB, customPromise, loadModel, plugins ,baseDir} = app.config.mongoose;
...
function loadModelToApp(app) {
  const dir = path.join(app.config.baseDir, 'app', baseDir||"model");
